...
beforeEach(() => {
  ...
  jest.mocked(useAgentSync).mockReturnValue({
    status: 'idle', actionsHydrated: true, groups: [], actions: [action()], getLogoutBlockingCount: jest.fn(), getAction: jest.fn(() => action()), syncNow: jest.fn(), retryNow,
  });
});
...
it('disables retry while it is running and hides it for non-retryable queue states', async () => {
  ...
  jest.mocked(useAgentSync).mockReturnValue({
    status: 'idle', actionsHydrated: true, groups: [], actions: [action('pending')], getLogoutBlockingCount: jest.fn(), getAction: jest.fn(() => action('pending')), syncNow: jest.fn(), retryNow,
  });
  await render(<QueuedActionScreen actionId="action-1" />);
...
it('hides out-of-scope action facts behind a neutral unavailable state', async () => {
  jest.mocked(useAgentSync).mockReturnValue({
    status: 'idle', actionsHydrated: true, groups: [], actions: [], getLogoutBlockingCount: jest.fn(), getAction: jest.fn(() => undefined), syncNow: jest.fn(), retryNow,
  });
